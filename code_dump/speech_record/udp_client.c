...
int main(void) {
  ...
  for (i = 0; i < numSamples; i++) {
    ...
    if (!data.recordedSamples[i]) {
      break;
    }

    sampleArr[i] = 0 + data.recordedSamples[i];
    printf("%d th  %f\n", i, sampleArr[i]);
  }
...
*/
  ...
  printf("Sending recorded audio data...\n");
  for (i = 0; i < numSamples; i++)
    printf("%d th %f\n", i, sampleArr[i]);

  // Calculate the number of floats we can send in each packet
  packet_size = MAX_PACKET_SIZE;
  num_floats = packet_size / sizeof(float);

  // Send the array in chunks
  for (i = 0; i < 440999; i += num_floats) {
    int remaining = 440999 - i;
    int floats_to_send = (remaining < num_floats) ? remaining : num_floats;

    sent_bytes = sendto(sockfd, &sampleArr[i], floats_to_send * sizeof(float),
  ...
  for (i = 0; i < 5; i++) {
    // Send the EOT code
    sent_bytes = sendto(sockfd, &code, sizeof(float), 0,
                        (const struct sockaddr *)&servaddr, sizeof(servaddr));
    if (sent_bytes < 0) {
      perror("Send EOT code failed");
      exit(EXIT_FAILURE);
    }
    sleep(1);
  }
  // sendto(sockfd, sampleArr, 1024, MSG_CONFIRM,
  // (const struct sockaddr *)&servaddr, sizeof(servaddr));
