...
WCHAR container_name[] = L"MtSandboxTest2";
...
{
     const wchar_t* wide_string = L"C:\\Windows\\System32\\notepad.exe";
    char buffer[256] = { 0 };
    size_t converted = 0;

    errno_t err = wcstombs_s(&converted, buffer, sizeof(buffer), wide_string, _TRUNCATE);
    if (err == 0) {
        std::cout << "Converted string: " << buffer << std::endl;
    }
    else {
        std::cerr << "Error converting wide-character string: " << err << std::endl;
    }
    RunExecutableInContainer(buffer);
